...
def _eval_chunk(chunk, threshs, embs, idx):
    tp_fp_fn = np.empty((threshs.shape[0], 3), dtype=np.int32)

    embs_idx1 = np.vectorize(idx.get)(chunk["item1"].values)
    embs_idx2 = np.vectorize(idx.get)(chunk["item2"].values)
    dis = np.linalg.norm(embs[embs_idx1] - embs[embs_idx2], axis=1).astype(np.float32)
...
def eval(config, total_pairs):
    ...
    for chunk in tqdm(df, total=(total_pairs//chunksize + 1)):
        tp_fp_fn += _eval_chunk(chunk, threshs, embs, idx)
